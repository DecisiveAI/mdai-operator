...
func (c HubAdapter) ensureVariableSynced(ctx context.Context) (OperationResult, error) {
	...
	for _, collector := range collectors {
		if _, shouldRestart := namespaceToRestart[collector.Namespace]; shouldRestart {
			...
			valkeyClient := *c.valKeyClient
			threshold := strconv.FormatInt(time.Now().UnixMilli()-valkeyAuditStreamExpiry.Milliseconds(), 10)
			if result := valkeyClient.Do(ctx, valkeyClient.B().Xadd().Key(mdaiHubEventHistoryStreamName).Minid().Threshold(threshold).Id("*").FieldValue().FieldValueIter(composeValkeyStreamIterFromMap(mdaiHubEvent)).Build()); result.Error() != nil {
